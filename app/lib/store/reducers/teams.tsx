...
export const picksUntilActiveTeam = (activeTeam: number, currentPick: number, numberOfTeams: number) => {
    const myRound = Math.floor(currentPick / numberOfTeams) + 1;
    const isForward = myRound % 2 === 1;

    let pickInRound;
    if (isForward) {
        pickInRound = activeTeam;
    } else {
        pickInRound = numberOfTeams - 1 - activeTeam;
    }

    const roundPick = currentPick % numberOfTeams;
    let v = pickInRound - roundPick;
    if (v >= 0) {
        return v;
    }

    const leftInRound = numberOfTeams - roundPick;
    let pickInNextRound;
    if (!isForward) {
        pickInNextRound = activeTeam;
    } else {
